...
['readFile', 'readdir', 'mkdir', 'rmdir', 'unlink'].forEach(item => {
  resultObj[item] = function (pathname, encoding = 'utf-8') {
...
['writeFile', 'appendFile'].forEach(item => {
  resultObj[item] = function (pathname, content, encoding = 'utf-8') {
    console.log("这里是 promisFS，会被访问吗？？ "+content);
    // 支持JSON类型数据
    (typeof content == 'object') ? content = JSON.stringify(content) : null
    // 将传入的内容转为 对象
...
resultObj['copyFile'] = function (pathname1, pathname2) {
  return new Promise((resolve, reject) => {
    ...
    console.log("这里是promisFS，");
    console.log("pathname1:   "+ pathname1 + "pathname2 :   "+ pathname2);
